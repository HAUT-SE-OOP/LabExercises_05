...
int printArray(const T *array,int size,int lowSubscrip,int highSubscript)
{
   ...
   if ( /* Write conditions to test if the size if negative,
           or if the range is invalid */lowSubscrip<0||highSubscript>=size||highSubscript<=lowSubscrip)
      return 0;
   ...
   for ( /* Write code to iterate from lowSubscript up to
            and including highSubscript */int i=lowSubscrip;i<=highSubscript;i++ )
   {
      ++count;
      cout << array[ i ] << ' ';
   } // end for
...
int main()
{
   ...
   elements = /* Write a call to printArray that specifies
                 0 to BCOUNT - 1 as the range */printArray(b,BCOUNT,0,BCOUNT-1);
   cout << elements << " elements were output\n";

   // display elements 1-3 of array b
   cout << "Array b from positions 1 to 3 is:\n";
   elements = /* Write a call to printArray that specifies
                 1 to 3 as the range */printArray(b,BCOUNT,1,3);
   cout << elements << " elements were output\n";

   // try to print an invalid element
   cout << "Array b output with invalid subscripts:\n";
   elements = /* Write a call to printArray that specifies
                 -1 to 10 as the range */printArray(b,BCOUNT,-1,10);
   cout << elements << " elements were output\n\n";
   ...
   elements = /* Write a call to printArray that specifies
                 0 to CCOUNT - 2 as the range */printArray(c,CCOUNT,0,CCOUNT-2);
   cout << elements << " elements were output\n";

   // display elements 1-3 of array c
   cout << "Array c from positions 1 to 3 is:\n";
   elements = /* Write a call to printArray that specifies
                 1 to 3 as the range */printArray(c,CCOUNT,1,3);
   cout << elements << " elements were output\n";

   // try to display an invalid element
   cout << "Array c output with invalid subscripts:\n";
   elements = /* Write a call to printArray that specifies
                 -1 to 10 as the range */printArray(c,CCOUNT,-1,10);
