...
// original function
template< typename T >
void printArray( const T *array, int count1 )
{
   // display array
    int i = 0; 
  while(i < count1 )
...
/* Write a header for an overloaded printArray function
   ...
   header */
template< typename T >
int printArray( const T *array, int count1, int lowSubscript, int highSubscript )
{
   // check if subscript is negative or out of range
   if ( /* Write conditions to test if the size if negative,
           or if the range is invalid */ !( highSubscript > lowSubscript && ( highSubscript < count1 || highSubscript == count1 ) && ( lowSubscript > 0 || lowSubscript == 0) ) )
      return 0;
   ...
   // display array
   int i = lowSubscript ;
   while ( /* Write code to iterate from lowSubscript up to
            and including highSubscript */  i <= highSubscript  )
   {i++;
      ++count2;
      cout << array[ i ] << ' ';
   } // end for
   ...
   cout << '\n';
   return count2; // number or elements output
...
int main()
{
   ...
   elements = (a,ACOUNT, -1, 10);/* Write a call to printArray that specifies
   ...
   elements = printArray( c , CCOUNT , 0, CCOUNT - 2 );/* Write a call to printArray that specifies
