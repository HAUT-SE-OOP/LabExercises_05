...
/* Write a header for an overloaded printArray function
   ...
   header */
template <typename T>
int printArray(const T *array,int c,int lowSubscript,int highSubscript)
{
   // check if subscript is negative or out of range
   if ( c<0 || highSubscript>c || lowSubscript<0 || lowSubscript>=highSubscript/* Write conditions to test if the size if negative,
