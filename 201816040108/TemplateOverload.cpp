...
/* Write a header for an overloaded printArray function
   ...
   header */
template< typename T >
int printArray(const T *array, int lowSubscrip,int highSubscrip)
{
   // check if subscript is negative or out of range
   if (lowSubscrip<0||highSubscrip<lowSubscrip/* Write conditions to test if the size if negative,
   ...
   // display array
   for ( int i=lowSubscrip;i<=highSubscrip;i++/* Write code to iterate from lowSubscript up to
...
int main()
{
   ...
   // display array b using original printArray functio
   cout << "\nUsing original printArray function\n";
   printArray( b, BCOUNT );
