...
/* Write a header for an overloaded printArray function
   ...
   header */
template< typename S>
int printArray(const S *array,int lowSubscrip,int highSubscript)
{
   // check if subscript is negative or out of range
   if (highSubscript>sizeof(array)||highSubscript<lowSubscrip||lowSubscrip<0 /* Write conditions to test if the size if negative,
...
int main()
{
  ...
  // display array a using original printArray function
   ...
   printArray(a,ACOUNT);
   elements=sizeof(a)/sizeof(a[0]); /* Write a call to printArray that specifies
   ...
   printArray( b, BCOUNT );
   elements =sizeof(b)/sizeof(b[0]); /* Write a call to printArray that specifies
   ...
   printArray( c, CCOUNT );
   elements =sizeof(c)/sizeof(c[0])-1;/* Write a call to printArray that specifies
