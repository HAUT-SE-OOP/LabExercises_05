...
// takes upper and lower subscripts to print
    /* Write a header for an overloaded printArray function
       ...
       header */
template< typename T >
int printArray(const T *array,int d,int lowSubcrip,int highSubcript)
{
                                                        // check if subscript is negative or out of range
   if ( lowSubcrip>highSubcript||highSubcript>d/* Write conditions to test if the size if negative,
   ...
   // display array
   for (int i=lowSubcrip;i<=highSubcript;i++/* Write code to iterate from lowSubscript up to
