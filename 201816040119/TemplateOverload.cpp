...
/* Write a header for an overloaded printArray function
   ...
   header */
   template< typename T >
int printArray(const T *array,int element,int lowSubscrip,int highSubscriptf)
{
   // check if subscript is negative or out of range
   if ( lowSubscrip<0||lowSubscrip>highSubscriptf||highSubscriptf>element)
      return 0;

   int count = 0;

   // display array
   for ( int i=lowSubscrip;i<=highSubscriptf;i++ )
   {
      ++count;
      cout << array[ i ] << ' ';
   } // end for
