...
/* Write a header for an overloaded printArray function
   ...
   header */
   template< typename P >
bool printArray1( const P *array, int highSubscript,int lowSubscript )
{
   // check if subscript is negative or out of range
   if ( lowSubscript>=highSubscript/* Write conditions to test if the size if negative,
   ...
   int i;
   // display array
   for (i=lowSubscript;i<=highSubscript;i++/* Write code to iterate from lowSubscript up to
...
int main()
{
   ...
   elements =ACOUNT ;/* Write a call to printArray that specifies
                 0 to ACOUNT - 1 as the range */
    printArray1(a,ACOUNT-1,0);
   cout << elements << " elements were output\n";

   // display elements 1-3 of array a
   cout << "Array a from positions 1 to 3 is:\n";
   elements = 3; /* Write a call to printArray that specifies
                 1 to 3 as the range */
    printArray1(a,3,1);
   cout << elements << " elements were output\n";
   ...
   elements = 0;/* Write a call to printArray that specifies
   ...
   elements =BCOUNT; /* Write a call to printArray that specifies
                 0 to BCOUNT - 1 as the range */
    printArray1( b,BCOUNT-1,0 );
   cout << elements << " elements were output\n";
   ...
   elements =CCOUNT; /* Write a call to printArray that specifies
                 0 to CCOUNT - 2 as the range */
    printArray1(c,CCOUNT,0);
   cout << elements << " elements were output\n";
