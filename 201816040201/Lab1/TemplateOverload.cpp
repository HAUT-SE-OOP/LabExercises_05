...
int printArray(const FK *array,int cnt,int intlowSubscript,int intHighSubscript)
{
   // check if subscript is negative or out of range
   if ( intHighSubscript<=intlowSubscript||intlowSubscript<0||intHighSubscript>=cnt )
      return 0;

   int count = 0;

   // display array
   for ( int i=intlowSubscript;i<=intHighSubscript;i++)
   {
      ++count;
      cout << array[ i ] << ' ';
   } // end for
