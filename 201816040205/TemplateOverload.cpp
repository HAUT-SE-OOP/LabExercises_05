...
template< typename T >
T printArray(const T *array, int size, int lowSubscript, int highSubscript )
{
   // check if subscript is negative or out of range
   if ( lowSubscript>highSubscript||lowSubscript<0||lowSubscript>=size||highSubscript>=size )
      return 0;
