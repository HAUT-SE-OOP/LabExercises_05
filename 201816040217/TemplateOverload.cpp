...
template<typename T>
int printArray( const T *array, int lowSubscript,int highSubscript,int maxSize)
{
   // check if subscript is negative or out of range
   if ( maxSize <=0 || highSubscript >= maxSize/* Write conditions to test if the size if negative,
           or if the range is invalid */ )
      return 0;

   int count = 0;
   if( highSubscript < 0)
        highSubscript = maxSize+highSubscript;
   // display array
   for ( int i = lowSubscript; i <= highSubscript; i++ /* Write code to iterate from lowSubscript up to
...
int main()
{
   ...
   elements = printArray( a,0,-1,ACOUNT);/* Write a call to printArray that specifies
   ...
   elements = printArray(a,1,3,ACOUNT);/* Write a call to printArray that specifies
                 1 to 3 as the range */
   cout << elements << " elements were output\n";

   // try to print an invalid element
   cout << "Array a output with invalid subscripts:\n";
   elements = printArray(a,-1,10,ACOUNT);/* Write a call to printArray that specifies
   ...
   elements = printArray(b,0,-1,BCOUNT);/* Write a call to printArray that specifies
                 0 to BCOUNT - 1 as the range */
   cout << elements << " elements were output\n";

   // display elements 1-3 of array b
   cout << "Array b from positions 1 to 3 is:\n";
   elements = printArray(b,1,3,BCOUNT);/* Write a call to printArray that specifies
                 1 to 3 as the range */
   cout << elements << " elements were output\n";

   // try to print an invalid element
   cout << "Array b output with invalid subscripts:\n";
   elements = printArray(b,-1,10,BCOUNT);/* Write a call to printArray that specifies
                 -1 to 10 as the range */
   cout << elements << " elements were output\n\n";
   ...
   elements = printArray(c,0,-2,CCOUNT);/* Write a call to printArray that specifies
                 0 to CCOUNT - 2 as the range */
   cout << elements << " elements were output\n";

   // display elements 1-3 of array c
   cout << "Array c from positions 1 to 3 is:\n";
   elements = printArray(c,1,3,CCOUNT);/* Write a call to printArray that specifies
                 1 to 3 as the range */
   cout << elements << " elements were output\n";

   // try to display an invalid element
   cout << "Array c output with invalid subscripts:\n";
   elements = printArray(c,-1,10,CCOUNT);/* Write a call to printArray that specifies
                 -1 to 10 as the range */
