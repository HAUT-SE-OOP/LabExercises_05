...
/* Write a header for an overloaded printArray function
   ...
   header */
template< typename T>
T printArray(const T* array,int tcount,int lowSubscrip, int highSubscript)
{
   // check if subscript is negative or out of range
   if ( lowSubscrip<0||highSubscript<0||lowSubscrip>highSubscript||(highSubscript-lowSubscrip+1)>tcount /* Write conditions to test if the size if negative,
