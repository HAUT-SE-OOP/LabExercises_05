...
/* Write a header for an overloaded printArray function
   ...
   header */
template<typename T>
int printArray( const T *array, int lowSubscript,int highSubscript,int maxx)
{
   // check if subscript is negative or out of range
   if ( maxx <=0 || highSubscript >= maxx/* Write conditions to test if the size if negative,
           or if the range is invalid */ )
      return 0;

   int count = 0;
   if( highSubscript < 0)
        highSubscript = maxx+highSubscript;
   // display array
...
int main()
{
   ...
   elements = printArray( a,0,-1,ACOUNT);
   ...
   elements =printArray(a,1,3,ACOUNT);
    /* Write a call to printArray that specifies
                 1 to 3 as the range */
   cout << elements << " elements were output\n";
   ...
   elements =printArray(a,-1,10,ACOUNT);
    /* Write a call to printArray that specifies
   ...
   elements = printArray(b,0,-1,BCOUNT);
   /* Write a call to printArray that specifies
   ...
   cout << "Array b from positions 1 to 3 is:\n";
     elements = printArray(b,1,3,BCOUNT);
   /* Write a call to printArray that specifies
                 1 to 3 as the range */
   cout << elements << " elements were output\n";
   ...
   cout << "Array b output with invalid subscripts:\n";
     elements = printArray(b,-1,10,BCOUNT);
   /* Write a call to printArray that specifies
                 -1 to 10 as the range */
   cout << elements << " elements were output\n\n";
   ...
   elements = printArray(c,0,-2,CCOUNT);
   /* Write a call to printArray that specifies
   ...
   elements = printArray(c,1,3,CCOUNT);
   /* Write a call to printArray that specifies
                 1 to 3 as the range */
   cout << elements << " elements were output\n";
   ...
   elements = printArray(c,-1,10,CCOUNT);
   /* Write a call to printArray that specifies
                 -1 to 10 as the range */
