...
template< typename T >
int printArray(const T *array, int low,int high,int n)/* Write a header for an overloaded printArray function
   ...
   header */
{
   ...
   if ( high>n||high<low/* Write conditions to test if the size if negative,
   ...
   // display array
   for ( int i=low; i<= high;i++/* Write code to iterate from lowSubscript up to
...
int main()
{
   ...
   elements = printArray( a, 0, ACOUNT-1,ACOUNT); /* Write a call to printArray that specifies
   ...
   elements = printArray( b, 0, BCOUNT - 1,BCOUNT);/* Write a call to printArray that specifies
   ...
   elements = printArray( c, 0, CCOUNT-2 ,CCOUNT);/* Write a call to printArray that specifies
