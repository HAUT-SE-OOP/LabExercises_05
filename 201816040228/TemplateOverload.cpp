...
int printArray( const T *array, int size , int lowSubscrip, int  highSubscript)
{
   ...
   // display array
   for (int i = lowSubscrip; i <= highSubscript; i++ )//iterate from lowSubscrip up to and including highSubscript
   {
      ++count;
      cout << array[ i ] << ' ';
   } // end for
...
int main()
{
   ...
   elements = printArray(a, 5, 0,ACOUNT - 1); // a call that printArray  specifies 0 to ACOUNT - 1 as the range
   cout << elements << " elements were output\n";

   // display elements 1-3 of array a
   cout << "Array a from positions 1 to 3 is:\n";
   elements = printArray(a, 5, 1, 3 );//  a call   that printArray specifies 1 to 3 as the range

   cout << elements << " elements were output\n";

   // try to print an invalid element
   cout << "Array a output with invalid subscripts:\n";
   elements = printArray(a, 5, -1, 10 );//a call  that printArray specifies -1 to 10 as the range
   ...
   elements = printArray(b, 7, 0, BCOUNT - 1 );// a call  that printArray specifies 0 to BCOUNT - 1 as the range

   cout << elements << " elements were output\n";

   // display elements 1-3 of array b
   cout << "Array b from positions 1 to 3 is:\n";
   elements = printArray(b, 7, 1, 3 );//  printArray that specifies 1 to 3 as the range

   cout << elements << " elements were output\n";

   // try to print an invalid element
   cout << "Array b output with invalid subscripts:\n";
   elements = printArray(b, 7, -1, 10 );// printArray that specifies  -1 to 10 as the range
   ...
   elements = printArray(c, 6, 0, CCOUNT - 2 );//printArray that specifies 0 to CCOUNT - 2 as the range

   cout << elements << " elements were output\n";

   // display elements 1-3 of array c
   cout << "Array c from positions 1 to 3 is:\n";
   elements = printArray(c, 6, 1, 3 );//printArray that specifies 1 to 3 as the range

   cout << elements << " elements were output\n";

   // try to display an invalid element
   cout << "Array c output with invalid subscripts:\n";
   elements = printArray(c, 6, -1, 10 );//printArray that specifies  -1 to 10 as the range
