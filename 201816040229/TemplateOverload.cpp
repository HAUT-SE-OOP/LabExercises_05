...
/* Write a header for an overloaded printArray function
   that takes two additional int arguments, lowSubscript
   and highSubscript; remember to include the template
   header */
template< typename T>
double printArray(const T*array, int a,int b,int c )
{
   // check if subscript is negative or out of range
   if ( a<0||b>c/* Write conditions to test if the size if negative,
   ...
   int count = 0;
    int i;
   // display array
   for (i=a;i<=b;i++ /* Write code to iterate from lowSubscript up to
...
int main()
{
   ...
   elements = printArray(b,0,ACOUNT-1,ACOUNT);/* Write a call to printArray that specifies
   ...
   elements =printArray(b,1,3,ACOUNT-1); /* Write a call to printArray that specifies
                 1 to 3 as the range */
   cout << elements << " elements were output\n";
   ...
   elements =   printArray(b,-1,10,ACOUNT-1); /* Write a call to printArray that specifies
                 -1 to 10 as the range */
   cout << elements << " elements were output\n\n";
   ...
   elements =printArray(c,0,ACOUNT-1,ACOUNT);/* Write a call to printArray that specifies
   ...
   elements = printArray(c,1,3,ACOUNT);/* Write a call to printArray that specifies
                 1 to 3 as the range */
   cout << elements << " elements were output\n";
   ...
   elements =    printArray(c,-1,10,ACOUNT-1);/* Write a call to printArray that specifies
                 -1 to 10 as the range */
