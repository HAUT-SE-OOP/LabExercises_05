...
/* Write a header for an overloaded printArray function
   ...
   header */
   template<typename T>
   int printArray(const T *array,int a,int lowSubscript,int highSubscript)
   /*{
...

{
   ...
   if (lowSubscript<0||highSubscript>=a/* Write conditions to test if the size if negative,
...
int main()
{
   ...
   elements =printArray( a, -1, 0); /* Write a call to printArray that specifies
   ...
   elements = printArray( b, 0,  - 1 );/* Write a call to printArray that specifies
   ...
   elements = printArray(c, CCOUNT, 0, CCOUNT - 1);/* Write a call to printArray that specifies
