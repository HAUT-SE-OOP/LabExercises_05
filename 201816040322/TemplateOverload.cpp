...
/* Write a header for an overloaded printArray function
   ...
   header */
template< typename T >
int printArray( const T *array, int lowSubscrip, int highSubscript )
{
   // check if subscript is negative or out of range
   if ( lowSubscrip < 0 || highSubscript < 0 )
   ...
   for ( int i = lowSubscrip; i <= highSubscript; ++i )
        /* Write code to iterate from lowSubscript up to
...
int main()
{
   ...
   elements = printArray( a, 0, ACOUNT - 1 );
   ...
   elements = printArray( b, 0, BCOUNT - 1 );
   /* Write a call to printArray that specifies
   ...
   elements = printArray( c, 0, CCOUNT - 2 );
   /* Write a call to printArray that specifies
